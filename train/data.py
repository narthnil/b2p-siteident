...
from scripts.get_value_from_osm_tag import TagMap

DATA_PATHS = {
    "rwanda": {
        "roads": "data/osm/rwanda/roads/rwanda-roads.tif",
        "waterways": "data/osm/rwanda/waterways/rwanda-waterways.tif",
        "population": "data/Rwanda_population_data_2020_UNadj_constrained.tif",
        "targets": "data/targets/Rwanda training data_AllSitesMinusVehicleBridges_21.11.05.shp",
    },
    "uganda": {
        "roads": "data/osm/uganda/roads/uganda-roads.tif",
        "waterways": "data/osm/uganda/waterways/uganda-waterways.tif",
        "population": "data/Uganda_population_data_2020_UNadj_constrained.tif",
        "targets": "data/targets/Uganda_TrainingData_3districts_ADSK.shp",
    },
}
...
class BridgesData(Dataset):
    ...
    def __init__(self, tile_size=0.1, country="rwanda", inputs=()):
        ...
        self.tiles = []
        # TODO: Have statically defined bounds
        with rasterio.open(sample_input_file) as src:
            bounds = src.bounds
            for tile_lat in np.arange(bounds.left, bounds.right, tile_size):
                for tile_lon in np.arange(bounds.bottom, bounds.top, tile_size):
                    self.tiles.append(
                        (tile_lat, tile_lon, tile_lat + tile_size, tile_lon + tile_size)
                    )
