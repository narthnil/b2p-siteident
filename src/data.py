...
def process_population_data(population_data):
    population_data_zero_no_data = np.where(
        population_data == -99999, -1, population_data)
    population_data_standardized = (
        population_data_zero_no_data - RWANDA_POPULATION_MEAN) / RWANDA_POPULATION_STD
...
class BridgesData(Dataset):
    def __init__(self, tile_size=5000, country="rwanda", inputs=()):
        ...
        self.input_files = {_input: DATA_PATHS.get(
            country).get(_input) for _input in inputs}
        ...
        with rasterio.open(sample_input_file) as src:
            ...
            for tile_lon in np.arange(bounds.left, bounds.right, tile_size):
                for tile_lat in np.arange(bounds.bottom, bounds.top, tile_size):
                    self.tiles.append(
                        (tile_lon, tile_lat, tile_lon +
                         tile_size, tile_lat + tile_size)
                    )
    ...
    def __getitem__(self, item):
        ...
        for input_type in self.input_files:
            ...
            with rasterio.open(input_file) as src:
                ...
                raster_processed = STANDARDIZATION_FUNCTIONS.get(
                    input_type)(raster)
        ...
        data_item = {"input": input_channels,
                     "target": int(target_exists_in_bounds)}
        return data_item
