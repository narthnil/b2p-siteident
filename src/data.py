...
class BridgeDataset(Dataset):
    def __init__(self, tile_size=300, use_rnd_pos=False, transform=False,
                 ...
                 gdf_path="./data/ground_truth/v2/"):
        assert tile_size in [300, 600, 1200], "Tile size not known"
        self.tile_size = tile_size
        with open(path.join(gdf_path, "train_{}.geojson".format(
                tile_size))) as f:
...
def get_dataloaders(batch_size: int, tile_size: int) -> Dict[str, DataLoader]:
    pass
    # return {"train": train_dl, "val": val_dl, "test_dl": test_dl}
